...
type Device interface {
	...
	// GetSalinity converts EC to mS/cm
	//
	// formula source: https://www.aqion.de/site/electrical-conductivity
	GetSalinity(temp float32) float32
	Configure()
}
...
func (d *device) GetSalinity(temp float32) float32 {
	m := d.findMedian(d.collectSamples())
	return d.calcElectricalConductance(d.adc2voltage(m), temp) * 1000
}
