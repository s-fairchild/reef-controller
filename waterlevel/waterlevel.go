...
func NewWaterLevelSensor(sPin, rPin m.Pin, mode m.PinMode, reservoirSensor bool) WaterLevel {
	return &waterLevel{
		sensorPin:  sPin,
		relayPin: rPin,
		sensorMode: mode,
...
func (w *waterLevel) InitWaterLevel() {
	...
	fmt.Printf("Pump flow rate is %f gallons per second", gps)
...
func (w *waterLevel) MonitorLevel() {
	...
	for {
		if !w.sensorPin.Get() {
			// TODO don't actuate pump if w is a reservoir sensor
...
func (w *waterLevel) actuatePumpRelay() {
	...
	if w.pumpDelay.Before(time.Now()) && !reservoirEmpty {
		volumePumped += gps
		w.relayPin.High()
